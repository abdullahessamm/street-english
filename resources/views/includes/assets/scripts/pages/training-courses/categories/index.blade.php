...
$('#categories').DataTable({
    ...
    processing : true,
    serverSide : true,
    ajax : "{{ route('ajax.training-courses-category.index') }}",
    columns : [
...
$("#createNewCategory").on('submit', function(e){
    ...
    $.ajax({
        ...
        url : "{{ route('ajax.training-course-category.create') }}",
        type : "POST",
...
$(document).on('keyup', '.updateCategoryName', function(e){
    ...
    $.ajax({
        url : "{{ route('ajax.training-course-category.update') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
...
$(document).on('click', '.deleteTrainingCourseCategory', function(e){
    e.preventDefault();
    ...
    $.ajax({
        ...
        url : "{{ route('ajax.training-course-category.delete') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
            "course_category_id" : course_category_id,
