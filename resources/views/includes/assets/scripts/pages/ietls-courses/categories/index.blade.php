...
$('#categories').DataTable({
    'iDisplayLength': 4,
    "language": {
        "emptyTable" : "لا يوجد فئات لدورات ielts في قاعدة البيانات",
    },
    processing : true,
    serverSide : true,
    ajax : "{{ route('ajax.ietls-courses-category.index') }}",
    columns : [
...
$("#createNewCategory").on('submit', function(e){
    ...
    $.ajax({
        ...
        url : "{{ route('ajax.ietls-course-category.create') }}",
        type : "POST",
...
$(document).on('keyup', '.updateCategoryName', function(e){
    ...
    var course_category_id = $(this).data("ietlscourse-category-id");
    var course_category_name = $(this).text();

    $.ajax({
        url : "{{ route('ajax.ietls-course-category.update') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
...
$(document).on('click', '.deleteIETLSCourseCategory', function(e){
    e.preventDefault();
    ...
    $.ajax({
        ...
        url : "{{ route('ajax.ietls-course-category.delete') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
            "course_category_id" : course_category_id,
